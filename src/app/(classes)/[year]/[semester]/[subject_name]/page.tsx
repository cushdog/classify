...
import { fetchData, fetchSubjectFullName } from "@/lib/Library Functions and Clients/commonFunctions";
        ...
        if (subject_name && year && semester) {
            const fetchSubjectData = async () => {
                const modifiedSearch = `${
                    typeof subject_name === "string" ? subject_name.toUpperCase() : subject_name
                } ${
            ...
            <main className="container mx-auto px-4 py-8">
                    ...
                    <table className="w-full bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden transition-colors duration-300">
                        ...
                        {filteredData?.map((course, index) => (
                            <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-300">
                                <td className="px-6 py-4 whitespace-nowrap text-gray-900 dark:text-gray-100">{`${course[2]} ${course[3]}`}</td>
                ...
                <div className="md:hidden space-y-4">
                    {filteredData?.map((course, index) => (
                        ...
                        >
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{`${course[2]} ${course[3]}`}</h2>
                            ...
                            <div className="flex justify-between items-center">
                                <Dialog>
                                    <DialogTrigger asChild>
                                        <Button variant="outline" size="sm" className="border-gray-500 text-gray-300 dark:border-gray-600 dark:text-gray-400">
                                    ...
                                    <DialogContent>
                                        <DialogHeader>
                                            <DialogTitle className="text-gray-900 dark:text-gray-100">{`${course[2]} ${course[3]}: ${String(
                                                course[4]
                                            )}`}</DialogTitle>
                                        </DialogHeader>
