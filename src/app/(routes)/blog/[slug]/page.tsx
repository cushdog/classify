...
import { getBlogPostBySlug, getCommentsByPostId, insertComment } from '@/db/Blog/operations';
import { IBlogPost, IBlogComment } from '@/db/Blog/schema';
...
const BlogPostPage = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      if (!slug) return;
      try {
        const postData = await getBlogPostBySlug(slug as string);
        setPost(postData);
        if (postData) {
