...
const SubjectDetails: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchDataAndUpdateState = async () => {
      ...
      const uniqueData = data.filter((item: Course[], index: number, self: Course[][]) =>
        index === self.findIndex((t) => (
          t[2] === item[2] && t[3] === item[3]
        ))
      );
...
const handleClassClick = (classNumber: string) => {
  ...
  router.push(
    `/class?class=${classNumber}&term=${encodeURIComponent(
      mostRecentTerm
    )}`
  );
...
const handleBack = () => {
  ...
  return (
    ...
    {subjectData && subjectData.map((classData, index) => (
      <Card key={index} className="max-w-md mx-auto mt-8 overflow-hidden bg-gradient-to-br from-blue-500 to-purple-600 text-white shadow-lg">
        ...
        <CardContent className="p-4 space-y-2">
          <p><strong>Description:</strong> {String(classData[5])}</p>
          <p><strong>Credit Hours:</strong> {String(classData[6])}</p>
          <p><strong>Average GPA:</strong> {String(classData[22])}</p>
        </CardContent>
