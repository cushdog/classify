...
export default function SearchPage() {
  ...
  const searchWithoutTerm = (searchQuery: string) => {
    ...
    try {
      if (isSubjectSearch) {
        router.push(`/subject?subject=${searchQuery}&term=${encodeURIComponent(
          mostRecentTerm
        )}`);
      } else {
        router.push(`/class?class=${formattedQuery}&term=${encodeURIComponent(
          mostRecentTerm
        )}`);
      }
    } catch (error) {
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="w-full max-w-2xl p-4 flex flex-col space-y-4">
          <div className="flex items-center space-x-2">
            <Input value={search} onChange={handleInputChange} onSubmit={() => searchWithoutTerm(search)} />
          </div>
          <Button
